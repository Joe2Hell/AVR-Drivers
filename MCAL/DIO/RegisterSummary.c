...
1- DDR (Data Direction Register)
    ...
    x- When a bit in the DDR register is set to 0, it configures the corresponding pin as an input pin. 
...
2- PORT (Port Output register) 
    ...
    x- When a bit in the PORT register is set to 0, it sets the corresponding output pin to a logic Low (often ground).
...
3- PIN (Port Input register)    
    ...
    x- If the pin is at a logic low level, the corresponding bit in the PIN register will be 0
...
4- SFIOR (Special Function Input Output Register)   
    ...
    x- we are only concerned with Bit number 2

    x- Bit 2 corresponds for PUD (Pull Up Disable)

    x- If PUD is set to logic low level, the internal pull ups are enabled
    x- If PUD is set to logic high level, the internal pull ups are disabled
